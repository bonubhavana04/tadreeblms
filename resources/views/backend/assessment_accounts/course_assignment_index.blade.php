    ...
    <script>
        ...
        function loadDataTable() {
            ...
            dataTableInstance.on('draw', function () {
                ...
                // Client-side progress filter after draw
                var filter = $('#progress_filter').val();
                if (filter) {
                    $('#myTable tbody tr').each(function() {
                        var cell = $(this).find('td').eq(9);
                        var pct = parseInt(cell.find('.badge').text()) || 0;
                        var show = false;
                        if (filter === '0')       show = (pct === 0);
                        else if (filter === '1-50')  show = (pct >= 1 && pct <= 50);
                        else if (filter === '51-99') show = (pct >= 51 && pct <= 99);
                        else if (filter === '100')   show = (pct === 100);
                        $(this).toggle(show);
                    });
                }
            });
